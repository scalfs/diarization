...
def get_STFTs(segs, time_segs):
    ...
    for i, seg in enumerate(segs):
        S = librosa.core.stft(y=seg, n_fft=config.nfft, win_length=int(
            config.window * sr), hop_length=int(config.hop * sr))
        S = np.abs(S) ** 2
        ...
        for j in range(0, S.shape[1], int(.12/config.hop)):  # 0.24 / 0.01 = 24.0
            ...
            if j + 24 < S.shape[1]:
                if len(STFT_couple) < 2:
                    ...
                    time_windows.extend(
                        [[window_onset-0.12, window_onset+0.12], [window_onset, window_onset+0.24]])
            else:
                break
...
def align_embeddings(embeddings, intervals):
    ...
    segment_intervals = []
    use_first_partition_onset = True
    for i, partition in enumerate(partitions):
        avg_embeddings[i] = np.average(
            embeddings[partition[0]:partition[1]], axis=0)

        partition_interval = intervals[partition[0]:partition[1]]
        # start of first partition (the second really, because of the previous window step)
        partition_onset_idx = 0 if use_first_partition_onset else 1
        interval_onset = partition_interval[partition_onset_idx][0]
        # end of last partition
        use_first_partition_onset = True if partition_interval[-1][1] - \
            interval_onset > 0.4 else False
        partition_offset_idx = -2 if use_first_partition_onset else -1
        interval_offset = partition_interval[partition_offset_idx][1]
        # segment interval for the partition
...
def main():
    ...
    # Fixing to 2 since we take 2 for each interval #utter_batch.shape[1]
    batch_size = 2
    # verification batch (time x batch x n_mel)
    verif = tf.placeholder(shape=[None, batch_size, 40], dtype=tf.float32)
    batch = tf.concat([verif, ], axis=1)
    # embedding lstm (3-layer default)
    with tf.variable_scope("lstm"):
        lstm_cells = [tf.contrib.rnn.LSTMCell(
            num_units=config.hidden, num_proj=config.proj) for i in range(config.num_layer)]
        lstm = tf.contrib.rnn.MultiRNNCell(
            lstm_cells)    # make lstm op and variables
        # for TI-VS must use dynamic rnn
        outputs, _ = tf.nn.dynamic_rnn(
            cell=lstm, inputs=batch, dtype=tf.float32, time_major=True)
    ...
    #     print(f'Audio id: {audio_id}')
        if extension == '.wav':
            ...
            logging.info(
                f'Wrong file type in {os.path.join(data_path, audio_file)}')
    ...
    #     train_cluster_ids = []
        ...
        for audio_id, audio_path in audio_files.items():
            ...
            for STFT_window in STFT_windows:
                ...
                embeddings_batch = sess.run(
                    embedded, feed_dict={verif: STFT_batch})
                embeddings = np.concatenate((embeddings, embeddings_batch))

            # Turn window-level embeddings to segment-level (400ms)
            aligned_embeddings, segment_intervals = align_embeddings(
                embeddings, time_windows)
